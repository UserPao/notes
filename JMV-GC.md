...
2. 可达性分析算法（主流）
   ...
   **从可达性分析算法看minor GC 扫描老年代的问题**
   
   ​		MinorGC的是当新生代不够时触发的GC算法，它的好处就是不用堆整个堆进行垃圾回收或者扫描，但是如果当老年代的对象引用到新生代的对象，在判断对象是否存活时，采用了GC Roots可达性分析，需要标记存活对象，照这个道理，JVM还是需要扫描老年代，也就是扫描整个堆了。这不前后矛盾了嘛。那么JVM是怎么做的
...
**G1允许用户自定义设置最大停顿时间。实现的原理是因为在回收老年代的region的时候，只回收垃圾过多的，假如超过了用户设置的时间了，就不回收了。在用户允许的时间内尽可能的回收**

- G1收集器对堆空间的划分

  ![1589955805271](JMV-GC.assets\1589955805271.png)
